...
extern unsigned char D_obj_p_11_TextStyle_00DarkBackgroundCheckbox[];
void obj_p_11_TextStyle_00DarkBackgroundCheckbox( Asm4d_globals *glob, tProcessGlobals *ctx, int32_t inNbExplicitParam, int32_t inNbParam, PCV inParams[], CV *outResult)
{
	CallChain c(ctx,D_obj_p_11_TextStyle_00DarkBackgroundCheckbox);
	if (!ctx->doingAbort) {
		{
			Obj t0;
			c.f.fLine=5;
