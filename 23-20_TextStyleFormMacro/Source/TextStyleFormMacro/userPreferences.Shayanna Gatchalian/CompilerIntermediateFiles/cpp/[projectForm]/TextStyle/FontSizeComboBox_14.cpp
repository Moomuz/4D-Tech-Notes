...
extern unsigned char D_obj_p_13_TextStyle_00FontSizeComboBox[];
void obj_p_13_TextStyle_00FontSizeComboBox( Asm4d_globals *glob, tProcessGlobals *ctx, int32_t inNbExplicitParam, int32_t inNbParam, PCV inParams[], CV *outResult)
{
	CallChain c(ctx,D_obj_p_13_TextStyle_00FontSizeComboBox);
	if (!ctx->doingAbort) {
		{
			Obj t0;
			c.f.fLine=8;
