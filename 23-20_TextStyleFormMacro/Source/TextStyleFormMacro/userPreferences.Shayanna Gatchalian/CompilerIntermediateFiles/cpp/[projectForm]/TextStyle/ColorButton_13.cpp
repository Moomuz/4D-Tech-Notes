...
extern unsigned char D_obj_p_6_TextStyle_00ColorButton[];
void obj_p_6_TextStyle_00ColorButton( Asm4d_globals *glob, tProcessGlobals *ctx, int32_t inNbExplicitParam, int32_t inNbParam, PCV inParams[], CV *outResult)
{
	CallChain c(ctx,D_obj_p_6_TextStyle_00ColorButton);
	if (!ctx->doingAbort) {
		Txt lhexvalue;
		Long lrgbInteger;
		Txt lstring;
