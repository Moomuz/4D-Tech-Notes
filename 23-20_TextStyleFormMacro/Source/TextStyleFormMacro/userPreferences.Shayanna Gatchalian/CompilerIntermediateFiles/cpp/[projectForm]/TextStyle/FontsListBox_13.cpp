...
extern unsigned char D_obj_p_12_TextStyle_00FontsListBox[];
void obj_p_12_TextStyle_00FontsListBox( Asm4d_globals *glob, tProcessGlobals *ctx, int32_t inNbExplicitParam, int32_t inNbParam, PCV inParams[], CV *outResult)
{
	CallChain c(ctx,D_obj_p_12_TextStyle_00FontsListBox);
	if (!ctx->doingAbort) {
		Long lindex;
